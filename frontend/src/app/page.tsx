...
function FocusCard({
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -10 }}
      className="group rounded-2xl border border-primary/10 bg-surface/70 p-8 shadow-sm backdrop-blur-md transition-shadow hover:shadow-xl hover:shadow-accent/10"
    >
      <div className="flex items-center justify-between">
        <span className="rounded-full bg-accent/10 px-3 py-1 text-xs font-semibold text-accent">
          {badge}
        </span>
        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-accent/10 text-accent transition-colors group-hover:bg-accent group-hover:text-on-accent">
          <Icon size={22} />
        </div>
      </div>
    ...
    <main className="bg-background">
      ...
      <section className="relative min-h-screen overflow-hidden bg-background">
        ...
        <div className="absolute inset-0">
          ...
          />
          {/* Readability overlay (theme inverse — always readable on photo) */}
          <div className="absolute inset-0 bg-gradient-to-r from-inverse/92 via-inverse/55 to-transparent" />
        </div>
        ...
        <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-6 pb-24 pt-20 md:px-12 md:pt-28 lg:px-20">
          ...
          >
            ...
            <motion.h1
              variants={reveal}
              className="mt-4 text-3xl font-bold leading-tight tracking-tight text-on-inverse sm:text-4xl md:text-5xl"
            >
            ...
            <motion.div variants={revealShort} className="mt-8 flex flex-wrap gap-3">
              <Link
                href="#core-focus"
                className="inline-flex items-center gap-2 rounded-lg bg-accent px-5 py-2.5 text-sm font-semibold text-on-accent transition hover:opacity-90"
              >
                Explore EnerGrid
                <ArrowRight size={15} />
              </Link>
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-on-inverse/30 px-5 py-2.5 text-sm font-semibold text-on-inverse transition hover:border-accent hover:text-accent"
              >
                View Pipeline
              </Link>
            </motion.div>
      ...
      <section className="bg-inverse py-16">
        <div className="mx-auto max-w-6xl px-4">
          ...
          >
            {stats.map((s) => (
              ...
              >
                <s.icon size={28} className="mx-auto text-accent" />
                <p className="mt-3 text-2xl font-extrabold text-on-inverse md:text-3xl">
                  {s.value}
                </p>
                <p className="mt-1 text-sm text-on-inverse/70">{s.label}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <section id="core-focus" className="bg-surface py-20">
        <div className="mx-auto max-w-6xl px-4">
          <motion.div
            variants={reveal}
            initial="hidden"
          ...
          >
            ...
            <p className="mx-auto mt-3 max-w-2xl text-base text-secondary">
          ...
          >
            {partners.map((name) => (
              <motion.div
                key={name}
                variants={revealShort}
                className="flex h-20 w-44 items-center justify-center rounded-xl border border-primary/10 bg-surface text-lg font-bold tracking-wide text-primary shadow-sm"
              >
                {name}
              </motion.div>
            ))}
      ...
      {/* ── Section 6: CTA ── */}
      <section className="bg-gradient-to-r from-accent/10 via-surface to-accent/5 py-20">
        <motion.div
          variants={reveal}
          initial="hidden"
          whileInView="show"
        ...
        >
          ...
          <Link
            href="/contact"
            className="mt-8 inline-flex items-center gap-2 rounded-xl bg-accent px-8 py-3.5 text-sm font-semibold text-on-accent shadow-lg shadow-accent/25 transition hover:opacity-90"
          >
            Get in Touch
            <ArrowRight size={16} />
          </Link>
